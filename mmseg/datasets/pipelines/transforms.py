...
class PadRIND(object):
    ...
    def _pad_seg(self, results):
        ...
        for key in results.get('seg_fields', []):
            results[key] = mmcv.impad(
                results[key].transpose(1,2,0),
                shape=(5,) + results['pad_shape'][:2],
                pad_val=self.seg_pad_val,
                padding_mode='symmetric'
            )
...
@PIPELINES.register_module()
class NormalizeRIND(object):
    """NormalizeRIND the image.
    ...
    def __call__(self, results):
        ...
        results['img'] = self.trans(results['img'])
        results['img_norm_cfg'] = dict(mean=self.mean, std=self.std, to_rgb=self.to_rgb)
        #results['gt_semantic_seg'] = torch.from_numpy(results['gt_semantic_seg']).float()
        results['gt_semantic_seg'] = self.gt_trans(results['gt_semantic_seg'])
        return results
...
@PIPELINES.register_module()
class RandomCropTrainHD5(object):
    """Random crop the image & seg.
    ...
    def crop_hd5(self, img, crop_bbox):
        """Crop from ``hd5``"""
        crop_y1, crop_y2, crop_x1, crop_x2 = crop_bbox
        img = img[:, crop_y1:crop_y2, crop_x1:crop_x2]
        return img
    ...
    def __call__(self, results):
        ...
        # crop semantic seg
        for key in results.get('seg_fields', []):
            results[key] = self.crop_hd5(results[key], crop_bbox)
