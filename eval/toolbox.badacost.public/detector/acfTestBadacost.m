function [miss,roc,gt,dt] = acfTestBadacost( varargin )
...
%   .savePath  - ['.'] path in which the detector is saved.
%
% OUTPUTS
...
% get parameters
dfs={ 'name','REQ', 'imgDir','REQ', 'gtDir','REQ', 'numClasses', 'REQ', 'pLoad',[], ...
  'thr',.5,'mul',0, 'reapply',0, 'ref',10.^(-2:.25:0), ...
  'lims',[3.1e-3 1e1 .05 1], 'show',0, 'savePath', '.'};
[name,imgDir,gtDir,numClasses,pLoad,thr,mul,reapply,ref,lims,show,savePath] = ...
  getPrmDflt(varargin,dfs,1);

% run detector on directory of images
bbsNm=fullfile(savePath, [name 'Dets.txt']);
if(reapply && exist(bbsNm,'file')), delete(bbsNm); end
if(reapply || ~exist(bbsNm,'file'))
  detector = load(fullfile(savePath, [name 'Detector.mat']));
  detector = detector.detector;
  imgNms = bbGt('getFiles',{imgDir});

  acfDetectBadacost( imgNms, detector, bbsNm );
end

% run evaluation using bbGt
[gt,dt] = bbGt('loadAll',gtDir,bbsNm,pLoad,1); % isMulticlass = 1;
[gt,dt] = bbGt('evalRes',gt,dt,thr,mul,1); % isMulticlass = 1;
